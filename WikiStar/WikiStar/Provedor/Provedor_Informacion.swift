...
class proveedorDeInfromacion{
    let url_de_informacion = "https://swapi.dev/api/species/"
    ...
    func obtener_especies(que_hacer_al_recibir: @escaping ([Especies]) -> Void) {
        ...
        let ubicacion = URL(string: "\(url_de_informacion)posts")!
        URLSession.shared.dataTask(with: ubicacion) {
                (datos, respuesta, error) in do {
                    if let especies_recibidas = datos{
                        ...
                        self.lista_de_especies = prueba_de_interpretacion_de_datos
